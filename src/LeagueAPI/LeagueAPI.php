...
namespace PandaScoreAPI\LeagueAPI;
...
use PandaScoreAPI\LeagueAPI\Definitions\AsyncRequest;
use PandaScoreAPI\LeagueAPI\Definitions\CallCacheControl;
use PandaScoreAPI\LeagueAPI\Definitions\FileCacheProvider;
use PandaScoreAPI\LeagueAPI\Definitions\ICacheProvider;
use PandaScoreAPI\LeagueAPI\Definitions\ICallCacheControl;
use PandaScoreAPI\LeagueAPI\Definitions\IPlatform;
use PandaScoreAPI\LeagueAPI\Definitions\IRateLimitControl;
use PandaScoreAPI\LeagueAPI\Definitions\IRegion;
use PandaScoreAPI\LeagueAPI\Definitions\MemcachedCacheProvider;
use PandaScoreAPI\LeagueAPI\Definitions\Platform;
use PandaScoreAPI\LeagueAPI\Definitions\RateLimitControl;
use PandaScoreAPI\LeagueAPI\Definitions\Region;
use PandaScoreAPI\LeagueAPI\Exceptions\GeneralException;
use PandaScoreAPI\LeagueAPI\Exceptions\RequestException;
use PandaScoreAPI\LeagueAPI\Exceptions\RequestParameterException;
use PandaScoreAPI\LeagueAPI\Exceptions\ServerException;
use PandaScoreAPI\LeagueAPI\Exceptions\ServerLimitException;
use PandaScoreAPI\LeagueAPI\Exceptions\SettingsException;
use PandaScoreAPI\LeagueAPI\Objects\IApiObjectExtension;
use PandaScoreAPI\LeagueAPI\Objects\ProviderRegistrationParameters;
use PandaScoreAPI\LeagueAPI\Objects\StaticData;
use PandaScoreAPI\LeagueAPI\Objects\TournamentCodeParameters;
use PandaScoreAPI\LeagueAPI\Objects\TournamentCodeUpdateParameters;
use PandaScoreAPI\LeagueAPI\Objects\TournamentRegistrationParameters;
...
class LeagueAPI
{
    ...
    public function __construct(array $settings, IRegion $custom_regionDataProvider = null, IPlatform $custom_platformDataProvider = null)
    {
        ...
        if (true == $this->getSetting(self::SET_STATICDATA_LINKING)) {
            $calls_caching_settings = $this->getSetting(self::SET_CACHE_CALLS_LENGTH, []);
            if (false == $this->getSetting(self::SET_CACHE_CALLS)
                || (is_array($calls_caching_settings) && (false == isset($calls_caching_settings[self::RESOURCE_STATICDATA]) || $calls_caching_settings[self::RESOURCE_STATICDATA] <= 0))
    ...
    protected function _setupBeforeCalls()
    {
        ...
        $this->beforeCall[] = function () {
            if ($this->getSetting(self::SET_CACHE_RATELIMIT) && false != $this->rlc) {
                if (false == $this->rlc->canCall($this->getSetting($this->used_key), $this->getSetting(self::SET_REGION), $this->getResource(), $this->getResourceEndpoint())) {
    ...
    protected function _setupAfterCalls()
    {
        ...
        $this->afterCall[] = function () {
            ...
            if ($this->getSetting(self::SET_SAVE_DUMMY_DATA, false) && false == file_exists($dummyData_file)) {
    ...
    protected function makeCall(string $overrideRegion = null, string $method = self::METHOD_GET): PromiseInterface
    {
        ...
        if (!$requestPromise && $this->getSetting(self::SET_USE_DUMMY_DATA, false)) {
            ...
            try {
                ...
                if (false == $this->getSetting(self::SET_SAVE_DUMMY_DATA, false)) {
     ...
     *
     *     @see https://developer.riotgames.com/api-methods/#tournament-v4
     * ==================================================================d=d=
     **/
    ...
    public function createTournamentCodes(int $tournament_id, int $count, TournamentCodeParameters $parameters)
    {
        ...
        if (false == in_array($parameters->pickType, self::TOURNAMENT_ALLOWED_PICK_TYPES, true)) {
        ...
        if (false == in_array($parameters->mapType, self::TOURNAMENT_ALLOWED_MAPS, true)) {
        ...
        if (false == in_array($parameters->spectatorType, self::TOURNAMENT_ALLOWED_SPECTATOR_TYPES, true)) {
    ...
    public function createTournamentProvider(ProviderRegistrationParameters $parameters)
    {
        ...
        if (false == in_array(strtolower($parameters->region), self::TOURNAMENT_ALLOWED_REGIONS, true)) {
     ...
     *
     *     @see https://developer.riotgames.com/api-methods/#tournament-stub-v4
     * ==================================================================d=d=
     **/
